...
EOF
  ...
  echo ""
  echo -e "   ${GREEN}✓ The wildcard (*) covers ALL subdomains!${NC}"
  echo -e "   ${CYAN}ℹ Cloudflare automatically provisions FREE SSL certs for *.${DOMAIN}${NC}"
  echo -e "   ${CYAN}ℹ No manual cert management needed - everything is automatic!${NC}"
  echo ""
  ...
  echo -e "   ${YELLOW}⚠ ONLY after confirming tunnel SSH works!${NC}"
  echo "   sudo ufw delete allow OpenSSH"
  echo "   sudo ufw delete allow 22/tcp"
  echo -e "   ${GREEN}✓ Result: Zero open ports - all access via Cloudflare Tunnel!${NC}"
  echo ""
